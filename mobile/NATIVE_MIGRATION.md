...
| 27 | Home shows **today** by product beside the month, in the same bar format, on both the admin and the employee dashboard — the month-to-date figure alone cannot answer "what have we booked today". One `productSection` prints all four blocks; the screen had the same twenty lines pasted per role | `/api/app/dashboard/` (`product_today`) | **v4.40.0** |
